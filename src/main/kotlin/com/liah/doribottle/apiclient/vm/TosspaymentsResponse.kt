...
data class TosspaymentsResponse(
    ...
    val status: TosspaymentsPaymentStatus?, // 결제 처리 상태
    ...
    val card: TosspaymentsCardResponse?,
    ...
    val cancels: List<TosspaymentsPaymentCancelResponse>?, // 결제 취소 이력
    val secret: String?, // 가상계좌 웹훅이 정상적인 요청인지 검증하는 값
    val type: TosspaymentsPaymentType?,
    val easyPay: Any?, // 간편결제 정보
    val country: String?, // 결제한 국가
    val failure: TosspaymentsPaymentFailureResponse?, // 결제 실패 정보
    val isPartialCancelable: Boolean?, // 부분 취소 가능 여부
    val receipt: TosspaymentsReceiptResponse?, // 발행된 영수증 정보
...
data class TosspaymentsCardResponse(
