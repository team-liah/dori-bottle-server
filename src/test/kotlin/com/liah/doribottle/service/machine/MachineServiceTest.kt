...
class MachineServiceTest : BaseServiceTest() {
    ...
    fun register() {
        ...
        assertThat(machine?.rentCupAmounts).isEqualTo(DoriConstant.RENT_CUP_AMOUNTS)
        assertThat(machine?.rentIceCupAmounts).isEqualTo(DoriConstant.RENT_ICE_CUP_AMOUNTS)
    ...
    fun registerTc2() {
        ...
        // when
        val id = machineService.register(MACHINE_NO, MACHINE_NAME, VENDING, address, location, 100, null, 10L, 20L)
        clear()
        ...
        assertThat(machine?.capacity).isEqualTo(100)
        assertThat(machine?.cupAmounts).isEqualTo(0)
        assertThat(machine?.rentCupAmounts).isEqualTo(10L)
        assertThat(machine?.rentIceCupAmounts).isEqualTo(20L)
        assertThat(machine?.state).isEqualTo(NORMAL)
    }
    ...
    fun get() {
        ...
        assertThat(machineDto.rentCupAmounts).isEqualTo(DoriConstant.RENT_CUP_AMOUNTS)
        assertThat(machineDto.rentIceCupAmounts).isEqualTo(DoriConstant.RENT_ICE_CUP_AMOUNTS)
    }

    @DisplayName("자판기 조회 TC2")
    @Test
    fun getTc2() {
        // given
        val address = Address("12345", "삼성로", null)
        val location = Location(37.508855, 127.059479)
        val machine = machineRepository.save(Machine(MACHINE_NO, MACHINE_NAME, COLLECTION, address, location, 100, null))
        clear()
        ...
        assertThat(machineDto.no).isEqualTo(MACHINE_NO)
        assertThat(machineDto.name).isEqualTo(MACHINE_NAME)
        assertThat(machineDto.type).isEqualTo(COLLECTION)
        assertThat(machineDto.address).isEqualTo(address.toDto())
        assertThat(machineDto.location).isEqualTo(location.toDto())
        assertThat(machineDto.capacity).isEqualTo(100)
        assertThat(machineDto.cupAmounts).isEqualTo(0)
        assertThat(machineDto.state).isEqualTo(NORMAL)
        assertThat(machineDto.rentCupAmounts).isNull()
        assertThat(machineDto.rentIceCupAmounts).isNull()
    }
    ...
    fun insertMachines() {
        machineRepository.save(
            Machine("0000001", MACHINE_NAME, VENDING, Address("00001", "삼성로", null), Location(37.508855, 127.059479), 100, null),
        )
        machineRepository.save(
            Machine("0000002", MACHINE_NAME, VENDING, Address("00002", "삼성로", null), Location(37.508955, 127.052479), 100, null),
        )
        machineRepository.save(
            Machine("0000003", MACHINE_NAME, VENDING, Address("00003", "삼성로", null), Location(37.508355, 127.051479), 100, null),
        )
        machineRepository.save(
            Machine("0000004", MACHINE_NAME, VENDING, Address("00004", "마장로", null), Location(37.508455, 127.053479), 100, null),
        )
        machineRepository.save(
            Machine("0000005", MACHINE_NAME, COLLECTION, Address("00005", "도산대로", null), Location(37.518855, 127.029479), 100, null),
        )
        machineRepository.save(
            Machine("0000006", MACHINE_NAME, VENDING, Address("00006", "도산대로", null), Location(37.503855, 127.059179), 100, null),
        )
    }
